defmodule RiotApi.CryptoTest do
  ...
  describe "round-trip tests" do
    test "encrypt -> decrypt = original for complex payloads" do
      test_cases = [
        ...
        # Nested maps lose their atom keys
        {%{mixed: %{
          string: "test",
          number: 42,
          boolean: true,
      ...
      for {original, expected} <- test_cases do
        encrypted = Crypto.encrypt_payload(original)
        decrypted = Crypto.decrypt_payload(encrypted)
    ...
    test "encrypt -> decrypt for nested maps" do
      test_cases = [
        # Nested maps lose their atom keys because they go through JSON
    ...
    test "sign -> verify = true for various payloads" do
      test_cases = [
        %{},
        %{"simple" => "string"},
        %{"number" => 42},
    ...
    test "encrypt -> sign -> verify -> decrypt complete flow" do
      original = %{
      ...
      # Step 2: Sign
      signed = Crypto.sign_payload(encrypted)
      ...
      # Top-level keys remain atoms, but nested maps become strings
      expected = %{
        user: %{"name" => "John Doe", "email" => "john@example.com"},
        preferences: %{
          "theme" => "dark",
